...
while ($true) {
    ...
    $proc  = if (Test-Path $ProcessedF) { (Get-Content $ProcessedF -EA SilentlyContinue | Where-Object { $_.Trim() -ne "" }).Count } else { 0 }
    ...
    # TASKS PENDENTES PARA CLAUDE LOCAL
    $pendentes = Get-ChildItem $InboxCl -Filter *.json -EA SilentlyContinue | Where-Object {
        try {
            $j = Get-Content $_.FullName -Raw | ConvertFrom-Json
            $j.status -in @("pending", "accepted")
        } catch { $false }
    }
    if ($pendentes) {
        Write-Host ""
        Write-Host "  [AGUARDANDO CLAUDE LOCAL]" -ForegroundColor Yellow
    ...
    # REPLIES RECEBIDOS (outbox_claude)
    ...
    if ($replies) {
        ...
        foreach ($f in $replies) {
            try {
                ...
                Write-Host ("  {0} | ciclo={1} | {2}" -f $j.reply_id, $j.cycle, $j.status) -ForegroundColor $color
            } catch {
    ...
    if (Test-Path $PollerLog) {
        ...
        Write-Host "  (poller nao iniciado ou log nao encontrado)" -ForegroundColor DarkGray
    }

    # BEEP ao receber novo reply completo
    $hasComplete = ($replies | Where-Object {
        try { (Get-Content $_.FullName -Raw | ConvertFrom-Json).status -eq "complete" } catch { $false }
    }).Count -gt 0
