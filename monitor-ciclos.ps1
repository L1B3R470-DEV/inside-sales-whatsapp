...
$host.UI.RawUI.WindowTitle = "Monitor OPENLAW"
$lastBeepState = $false
...
function Write-Log($line) {
    ...
    elseif ($line -match "ACK|Subtarefa|iniciado|detectado|Processando") { Write-Host "  $line" -ForegroundColor Yellow }
...
while ($true) {
    ...
    Write-Host "  MONITOR OPENLAW AUTONOMOUS MODE" -ForegroundColor Cyan
    ...
    $done   = if ($worker -and $worker.processed_tasks) { $worker.processed_tasks.Count } else { 0 }
    ...
    Write-Host ""
    Write-Host "  [OPENCLAW workspace-integration]" -ForegroundColor White
    ...
    $ptDone = if (Test-Path $PtFile) { (Get-Content $PtFile -EA SilentlyContinue | Where-Object { $_.Trim() -ne "" }).Count } else { 0 }
    Write-Host ("  inbox_claude={0}  outbox_claude={1}  tasks_processadas={2}" -f $wsIn, $wsOut, $ptDone) -ForegroundColor Cyan

    # LOG AUTOPILOT
    Write-Host ""
    Write-Host "  [autopilot.log]" -ForegroundColor DarkGray
    ...
    # LOG RELAY-LOCAL
    Write-Host ""
    Write-Host "  [relay-local.log]" -ForegroundColor DarkGray
    if (Test-Path $RelayLog) {
        Get-Content $RelayLog -Tail 8 -EA SilentlyContinue | ForEach-Object { Write-Log $_ }
    } else {
        Write-Host "  (relay-local nao iniciado)" -ForegroundColor DarkGray
    }

    # TASKS PENDENTES
    $pendentes = Get-ChildItem $WsInbox -Filter *.json -EA SilentlyContinue | Where-Object {
        try {
            $j = Get-Content $_.FullName -Raw | ConvertFrom-Json
            $processados = if (Test-Path $PtFile) { Get-Content $PtFile -EA SilentlyContinue } else { @() }
            ($j.status -in @("pending","accepted")) -and ($j.task_id -notin $processados)
        } catch { $false }
    }
    if ($pendentes) {
        Write-Host ""
